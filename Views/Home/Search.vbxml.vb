            ...
            <ul>
                <div class="row">
                    ...
                    <div class="col-md-9" id="loaded">
                        <h2 fff="">Results for search query "<%= ViewData!query %>"</h2>
                                               ...
                                               WAS <span class="strike-text">KSh. <%= Format((spare.Price * 1.1), "#,#.00") %></span>
                                                       ...
                                                       <div class="d-flex flex-column mt-4">
                                                           ...
                                                           <%= (Function() As XElement
                                                                    Return If(Not String.IsNullOrWhiteSpace(Session.User) AndAlso (Session.Role = Session.Roles.SPARE_PARTS_DEALER Or Session.Role = Session.Roles.ADMIN) AndAlso (Session.User = spare.Username Or Session.Role = Session.Roles.ADMIN),
                                                                <div>
                                                                    <a class="btn btn-info" asp-controller="Home" asp-action="Edit" asp-route-id=<%= spare.ID %>>Modify </a>
                                                                    <input type="hidden" id="id" name="id" value=<%= spare.ID %>/>
                                                                    <input type="hidden" id="username" name="username" value=<%= spare.Username %>/>
                                                                    <input class="btn btn-danger" type="submit" value="Delete" id="deleteListing" style="width: 100%;"/>
                                                                </div>, Nothing)
                                                                End Function)() %>
                ...
                <ul class="pagination">
                    <%= (Function() As XElement
                             ...
                             <li class="page-item">
                                 <a class="page-link" href=<%= RequestBuilder("/Search?", ViewData!getStr, Page.PREV) %>>Previous</a>
                             </li>, Nothing)
                         End Function)() %>
                    ...
                    <%= (Iterator Function() As IEnumerable(Of XElement)
                             For Each p As Integer In Enumerable.Range(CInt(ViewData!from), CInt(ViewData!to))
                                 Yield <li class="page-item nums">
                                           <%= (Function()
                                                    Return If(p <> 0,
                                                    <a class="page-link" href=<%= RequestBuilder("/Search?", ViewData!getStr, Page.ELSE, p) %>><%= p %></a>, Nothing)
                                                End Function)() %>
                                       </li>
                             Next p
                         End Function)() %>
                    ...
                    <%= (Function() As XElement
                             ...
                             <li class="page-item">
                                 <a class="page-link" href=<%= RequestBuilder("/Search?", ViewData!getStr, Page.NEXT) %>>Next</a>
                             </li>, Nothing)
                         End Function)() %>
                </ul>
                <script>
                    $(document).ready(function() {
                        manageListings(<%= String.Format("""{0}""", Session.User) %>);
                        ...
                        doFilter("/Search?");
                        document.querySelector("input.form-control.me-2").value = "<%= ViewData!query %>";
                    });
                </script>
            </ul>
            <style>
